...
\paragraph{Vocabulary Extension via Quantized Embedding Surgery.}
...
This reduces N'Ko token count by 29.6\% (1,955 $\to$ 1,376 tokens on 30 eval examples) while preserving finite logits on both English and N'Ko inputs.
After a 500-iteration LoRA fine-tuning pass to learn the new embeddings, we expect the perplexity gap to close further.
...
\item The vocabulary extension (250 BPE tokens) requires additional fine-tuning to learn the new embeddings; constituent-mean initialization alone produces high perplexity on the new tokens.
