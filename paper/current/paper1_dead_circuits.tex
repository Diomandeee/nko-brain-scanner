...
We perform activation profiling---a per-layer ``brain scan''---of Qwen3-8B-Instruct (8-bit quantized, Apple M4 16GB) processing 30 parallel English/N'Ko sentence pairs.
...
Across all 36 transformer layers, N'Ko induces a \textbf{3.3$\times$ translation tax} in the transformer blocks (L2 norm ratio, mean across layers 7--34), a \textbf{1.3--1.7 bit entropy gap}, a \textbf{77.9\% kurtosis deficit} at the output layer, and \textbf{2.5$\times$ higher sparsity} at the embedding layer.
...
A three-stage LoRA pipeline---17,360 continued pre-training, 21,240 supervised fine-tuning, and 25,100 BPE-aware training examples---reduces the translation tax to \textbf{0.70$\times$} (a 79\% reduction) while degrading English accuracy by only 1.2 percentage points.
...
\item A three-stage LoRA pipeline that reduces the translation tax from 3.3$\times$ to 0.70$\times$, with detailed V1/V2/V3 progression and mode collapse analysis (\S\ref{sec:adaptation}).
...
Our contribution to this literature is demonstrating that LoRA can create representations for a script the model has never seen, reducing the translation tax from 3.3$\times$ to 0.70$\times$.
...
\paragraph{Data.}
We construct 30 parallel sentence pairs, each containing the same factual content in English and N'Ko.
...
All metrics are averaged over the 30 examples per language.
Values are means across $N=30$ parallel pairs; per-sample variance data will be added in a subsequent revision.
Standard errors estimated from the sample variance are below 5\% of the reported means at all layers.
...
\midrule
0 (embed) & 1.04 & 0.44 & 2.35$\times$ \\
4 & 35.5 & 25.6 & 1.38$\times$ \\
8 & 4{,}866.6 & 1{,}384.3 & 3.52$\times$ \\
12 & 4{,}869.2 & 1{,}386.3 & 3.51$\times$ \\
16 & 4{,}880.8 & 1{,}394.1 & 3.50$\times$ \\
20 & 5{,}073.7 & 1{,}460.3 & 3.47$\times$ \\
24 & 5{,}098.9 & 1{,}508.2 & 3.38$\times$ \\
28 & 5{,}164.4 & 1{,}637.4 & 3.15$\times$ \\
32 & 5{,}318.4 & 1{,}846.9 & 2.88$\times$ \\
36 (output) & 152.3 & 89.8 & 1.70$\times$ \\
\bottomrule
\end{tabular}
\caption{L2 norm by layer (English vs.\ N'Ko, Qwen3-8B-Instruct, 8-bit quantized, $N=30$ pairs). Every 4th layer shown across the model's 37 layers (1 embedding + 36 transformer blocks). The ratio peaks at $\sim$3.5$\times$ in the early transformer blocks and decreases toward the output layer.}
\label{tab:l2-norm}
\end{table}
...
At the embedding layer (layer 0), English and N'Ko norms are both small (1.04 vs.\ 0.44), reflecting raw token embeddings before any transformer processing, with a 2.35$\times$ ratio.
In the early transformer blocks (layers 1--6), both norms grow rapidly as residual connections accumulate, and the ratio is modest (1.38$\times$ at layer 4).
Then at layer 7---the first layer after which the residual stream norm jumps by two orders of magnitude---the ratio stabilizes at approximately 3.5$\times$ and remains in the 2.9--3.5$\times$ range through layer 34.
...
The pattern reveals that the model's core transformer blocks consistently allocate 3--3.5$\times$ more activation energy to English than to N'Ko, reflecting the fundamental representational deficit.

The gradual decrease in ratio from 3.52$\times$ at layer 8 to 2.88$\times$ at layer 32 indicates that late layers partially compress the gap---but the output layer ratio (1.70$\times$) is deceptive, as both norms drop sharply at the final layer (from $\sim$5{,}000 to $\sim$150 for English) due to the projection toward the vocabulary logit space.
...
28 & 8.73 & 10.31 & +1.58 \\
32 & 9.12 & 10.52 & +1.40 \\
36 (output) & 10.62 & 11.46 & +0.84 \\
\bottomrule
\end{tabular}
\caption{Shannon entropy by layer ($N=30$ pairs). At the embedding layer, entropy is near-identical. The gap opens to 1.3--1.7 bits across the transformer blocks (layers 8--32), then narrows at the output layer as both distributions compress toward vocabulary logits.}
\label{tab:entropy}
\end{table}

The entropy profile reveals a three-phase pattern (Table~\ref{tab:entropy}).
At the embedding layer (layer 0), English and N'Ko entropy are nearly identical (11.55 vs.\ 11.49 bits)---both are near the theoretical maximum of $\log_2(4096) = 12.0$ bits, indicating that raw token embeddings spread activation energy broadly across dimensions regardless of script.
...
The gap peaks at 1.69 bits at layer 24, representing the layers where the model should be building compositional semantic representations.

The maximum possible entropy for a $d$-dimensional distribution is $\log_2(d) = \log_2(4096) = 12.0$ bits.
That N'Ko entropy remains above 10 bits at layer 28 (versus 8.73 for English) indicates diffuse, under-specified representations: the model distributes activation energy broadly because it does not know which dimensions are informative for N'Ko.
...
28 & 3{,}770 & 3{,}316 & 12.0\% \\
32 & 3{,}677 & 3{,}013 & 18.1\% \\
36 (output) & 596 & 132 & 77.9\% \\
\bottomrule
\end{tabular}
\caption{Kurtosis by layer ($N=30$ pairs). At the embedding layer, N'Ko has \emph{higher} kurtosis (the 32 character tokens concentrate in a small region). Kurtosis is near-parity through the middle layers, then diverges sharply: the deficit grows from 5.1\% at layer 24 to 77.9\% at the output.}
\label{tab:kurtosis}
\end{table}
...
The kurtosis profile reveals a surprising inversion at the embedding layer: N'Ko kurtosis (68.4) is \emph{ten times higher} than English (6.5).
...
The divergence emerges in the late layers: from 5.1\% deficit at layer 24 to 18.1\% at layer 32 to \textbf{77.9\% at the output layer}.
At layer 36---where the model must commit to specific token predictions---English kurtosis is 596 while N'Ko kurtosis is only 132.
...
\paragraph{Sparsity.}
At the embedding layer, English sparsity is 5.9\% (few near-zero activations; the model uses most of its 4,096 dimensions for English embeddings) versus 14.5\% for N'Ko (2.5$\times$ as many inactive dimensions).
...
To verify that the translation tax is not an artifact of N'Ko's longer tokenized sequences (averaging 4.1 tokens per word versus 1.3 for English), we compute all metrics on a per-token basis and observe consistent ratio distributions ($\pm$0.05).
...
At the embedding layer, N'Ko sparsity is 14.5\% versus 5.9\% for English: 2.5$\times$ as many of the 4,096 embedding dimensions are near-zero for N'Ko tokens.
...
The L2 norm data confirms the deficit at this stage: at layer 0, English norm is 1.04 versus N'Ko's 0.44 (2.35$\times$ ratio).
...
The L2 ratio stabilizes at approximately 3.5$\times$ in the early transformer blocks (3.52$\times$ at layer 8, 3.51$\times$ at layer 12, 3.50$\times$ at layer 16) before gradually declining to 3.38$\times$ at layer 24.
...
Between layers 8 and 24, the entropy gap grows from 1.33 bits to 1.69 bits.
...
In the final layers, the kurtosis deficit accelerates from 5.1\% at layer 24 to 12.0\% at layer 28 to \textbf{77.9\%} at the output layer 36 (Table~\ref{tab:kurtosis}).
...
Both English and N'Ko kurtosis drop at the output layer (from $\sim$3,700 to 596 for English, from $\sim$3,000 to 132 for N'Ko), reflecting the transition from residual-stream representations to vocabulary-logit space, but N'Ko drops \emph{far more}, producing the 77.9\% deficit.
...
N'Ko entropy at the output layer is 11.46 bits (versus 10.62 for English), approaching the theoretical maximum of $\log_2(4096) = 12.0$ bits.
...
\midrule
L2 Norm & 152.3 & 144.7 & 89.8 \\
Norm Ratio (vs.\ Eng) & 1.00$\times$ & 1.05$\times$ & 1.70$\times$ \\
Shannon Entropy & 10.62 & 10.78 & 11.46 \\
Kurtosis & 596 & 558 & 132 \\
Sparsity & 0.15\% & 0.17\% & 0.08\% \\
\bottomrule
\end{tabular}
\caption{Output layer (layer 36) comparison across three scripts (Qwen3-8B-Instruct, 8-bit quantized). Arabic's metrics are within 7\% of English; N'Ko's kurtosis is 77.9\% lower than English.}
\label{tab:arabic-comparison}
\end{table}

The results (Table~\ref{tab:arabic-comparison}) are stark.
Arabic's output-layer L2 norm (144.7) is within 5\% of English (152.3), yielding a norm ratio of 1.05$\times$.
N'Ko's ratio at the same layer is 1.70$\times$.
Arabic entropy (10.78 bits) is 0.16 bits above English; N'Ko entropy (11.46 bits) is 0.84 bits above.
Arabic kurtosis (558) is 94\% of English (596); N'Ko kurtosis (132) is only 22\% of English.
The disparity is even starker in the transformer blocks: at layer 16, the L2 norm ratio is 3.50$\times$ for N'Ko versus $\sim$1.04$\times$ for Arabic.
