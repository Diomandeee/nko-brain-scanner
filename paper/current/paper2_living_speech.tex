...
\textbf{V3}: A Transformer CTC decoder (46.9M parameters) with 6 layers, 768 hidden dimension, and 4$\times$ downsampling on frozen Whisper features achieves 33\% validation CER and 70\% validation WER at epoch 200 (measured on a 10\% holdout from the bam-asr-early training split, not an independent test set).
...
Qualitatively, V4 produces coherent Bambara word sequences where V3 produces degenerate repetitions; quantitatively, V4 boosts prediction confidence from 0.46 to 0.82 (79\% improvement) and reduces WER from 70\% to 62.3\% ($-$11.0\% relative).
...
The script has 27 base characters occupying Unicode block \texttt{U+07C0--U+07FF} (standardized 2006) \cite{unicode2006nko}, writes right-to-left, and was engineered with a property that no evolved writing system possesses: a strict bijection between phonemes and graphemes.
...
\item \textbf{V4 Whisper LoRA results}: 30 epochs on A100, validation loss 0.884 $\to$ 0.290, with per-sample evaluation showing LoRA wins on 40\% of samples (sign test $p = 0.44$, not significant for WER) and qualitative shift from degenerate character repetition to coherent Bambara word production (\S\ref{sec:v4}).
...
The \emph{effective phonemic alphabet} for Bambara is 30 base characters (7 vowels + 23 consonants) plus combining diacritics---and each character corresponds to exactly one phoneme, meaning the CTC decoder need only learn 30 character-level emission patterns plus diacritic attachment rules.
...
The CTC output space is 66 classes: 64 N'Ko Unicode codepoints (\texttt{U+07C0--U+07FF}, covering 10 digits, 7 vowels, 23 consonants, 3 extended letters, 11 combining diacritics, and 10 punctuation/symbols) plus space and one CTC blank token.
...
The BiLSTM \cite{hochreiter1997lstm} lacks sufficient temporal modeling capacity for long-range phoneme context in connected speech.
...
We systematically vary four dimensions: architecture family (BiLSTM \cite{hochreiter1997lstm}, Transformer \cite{vaswani2017attention}, Conformer \cite{gulati2020conformer}), hidden dimension ($d \in \{256, 512, 768\}$), depth ($L \in \{2, 4, 6\}$ layers), and temporal downsampling ($\{4\times, 8\times, 16\times\}$).
...
\bottomrule
\end{tabular}
\caption{Full 28-configuration architecture search. Configurations sorted by family. \textbf{Bold}: V2 winner (Transformer $d{=}512$, $L{=}4$, 4$\times$), which is scaled to $d{=}768$, $L{=}6$ for V3 (\#23 achieves 38.2\% with 4 layers; V3 with 6 layers achieves 33\% validation CER).}
...
\item \textbf{6 Transformer layers} (up from 4): adds representational depth without proportionally increasing computation. The feed-forward dimension is $4d = 3072$ following standard practice \cite{vaswani2017attention}.
...
200 & 0.312 & 0.287 & \textbf{33\% val CER, 70\% val WER} \\
\bottomrule
\end{tabular}
\caption{V3 training progression. The model transitions from single-character repetition (epoch 1) to multi-word accuracy with tone diacritics (epoch 200) over 200 epochs. CER and WER at epoch 200 are measured on the training validation split (10\% holdout from bam-asr-early), not an independent test set.}
...
\paragraph{V3 result: 33\% validation CER, 70\% validation WER.}
These figures are measured on the training validation split (a 10\% holdout from the bam-asr-early corpus used during training), not an independent test set.
...
V4 partially unfreezes the Whisper encoder using LoRA \cite{hu2022lora}, allowing the upper encoder layers to adapt their acoustic representations to Bambara-specific phonemic patterns.
...
\bottomrule
\end{tabular}
\caption{V4 vs.\ V3 evaluation on 50 held-out test samples. V3 CER/WER figures (33.0\%/70.0\%) originate from training validation metrics at epoch 200 and are reproduced here on the same 50-sample test set for direct comparison. WER improves by 11\% relative, and prediction confidence nearly doubles.}
...
Base wins (lower WER) & 19 (38\%) \\
Tie (equal WER) & 11 (22\%) \\
\bottomrule
\end{tabular}
\caption{Per-sample comparison on 50 test samples. LoRA wins on 40\% of samples, with ties on 22\%.}
\label{tab:v4-persample-summary}
\end{table}

The per-sample comparison (Table~\ref{tab:v4-persample-summary}) shows that LoRA wins on 20/50 samples (40\%), base wins on 19/50 (38\%), and 11/50 (22\%) tie.
...
\bottomrule
\end{tabular}
\caption{Architecture evolution summary. CER/WER for V1--V3 are validation metrics (10\% holdout from training split); V4 metrics are measured on the same 50-sample test set as V3 for direct comparison. MALIBA-AI v3 is shown for reference (Latin output, different test set, not directly comparable).}
...
\item $\Sigma = C \cup V \cup T \cup N \cup \{\text{space}, \text{punct}\}$, with $C$ = N'Ko consonants (23), $V$ = N'Ko vowels (7), $T$ = tone diacritics (5), $N$ = nasalization marks (2)
...
Onset & $c \in C$ & Onset & Compound onset (rare)\footnotemark \\
Onset & sp/punct & reject & C without nucleus \\
\midrule
Nucleus & $t \in T$ & Nucleus & Tone attaches \\
Nucleus & $n \in N$ & Nucleus & Nasal mark attaches \\
Nucleus & $v \in V$ & Nucleus & New V-initial syllable \\
...
\end{table}
\footnotetext{In the implementation, consecutive consonants in the Onset state remain in Onset rather than rejecting, because some N'Ko consonant characters represent inherent compound onsets (e.g., \texttt{U+07DC} for /gb/). Similarly, consecutive vowels transition to a new Nucleus rather than rejecting, reflecting Manding vowel hiatus across syllable boundaries. Nasal combining marks (\texttt{U+07F2}, \texttt{U+07F3}) are treated as attachments to the Nucleus (like tone marks) rather than triggering a transition to a separate Coda state; the Coda state in the formal specification exists for completeness but the implementation treats the $(C)V(N)$ template by keeping nasal marks in the Nucleus state.}
...
\item NLLB-200 inference: 67ms per sentence (model inference only, excluding tokenization and network overhead; mean over 128-token sequences, FP16 on A100)
...
\item \textbf{Mac5}: Receives frame representations, runs CTC decoder (46.9M params, approximately 40ms), applies FSM post-processing ($<$1ms), optionally runs NLLB-200 for translation (67ms model inference).
\end{enumerate}
...
In companion work, we demonstrate that the translation tax imposed by N'Ko's unfamiliar script is universal across model families: 2.94$\times$ on Qwen3-8B, 3.22$\times$ on Qwen2.5-7B, and 3.42$\times$ on Mistral-7B \cite{diomande2026dead}.
...
Per-sample analysis shows V4 regresses on 38\% of test samples (19/50), primarily short, simple utterances.
...
Per-sample analysis shows V3 outperforms V4 on 38\% of test samples.
...
\bibitem[{Diomande(2026)}]{diomande2026dead}
