...
Per-sample evaluation shows LoRA wins on 20/50 samples, base wins on 17/50, and 13/50 tie, with dramatic improvements on worst-case inputs (sample au30: WER 15.8 $\to$ 1.0).
...
A two-tailed sign test on the 37 non-tied pairs does not reach significance ($p = 0.74$, Binomial test with $H_0$: $p = 0.5$), indicating that the per-sample WER improvement should be interpreted cautiously.
