...
\section{From Dead Circuits to Living Speech: Activation Profiling and
Script-Native ASR for
N'Ko}\label{from-dead-circuits-to-living-speech-activation-profiling-and-script-native-asr-for-nko}

\textbf{Mohamed Diomande} Independent Researcher
...
\textbf{Thread 1 (Diagnostic):} We perform activation profiling --- a
...
entropy inflation, 85.8\% kurtosis deficit at the output layer, and
150\% higher sparsity at embedding. Circuit duplication analysis (55
...
\textbf{Thread 2 (Solution):} We build the first audio-to-N'Ko ASR
...
downsampling, achieving 33\% CER and 70\% WER on 37 hours of Bambara
speech from bam-asr-early (CC-BY-4.0). A 4-state finite-state machine
encoding N'Ko syllable phonotactics guarantees 100\% structural
...
published Bambara ASR system --- MALIBA-AI bambara-asr-v3 (45.73\% WER),
...
\subsubsection{2.1 Bambara and Manding
ASR}\label{bambara-and-manding-asr}
...
WER on the MALIBA-AI benchmark corpus and 13.23\% WER under normalized
evaluation \cite{maliba2024bambara}. The sudoping01/bambara-asr-v2 model
achieves 25.07\% WER on its test split using a different data partition.
...
of Bambara ASR \cite{bambara_survey_2026} catalogues 11 publicly
available models, all targeting Latin-script output.

The Bayelemabaga corpus \cite{bayelemabaga2025} provides 46,976
Bambara-French parallel segments, and the WMT 2023 N'Ko shared task
\cite{wmt2023nko} established NMT baselines for N'Ko script (30.83
...
\subsubsection{2.2 Low-Resource ASR}\label{low-resource-asr}

The standard recipe for low-resource ASR is transfer learning from large
pre-trained acoustic models: Whisper \cite{radford2023robust}, wav2vec
2.0 \cite{baevski2020wav2vec}, and HuBERT \cite{hsu2021hubert}. These
...
\citet{graves2006connectionist} as a method for labeling unsegmented
...
SpecAugment \cite{park2019specaugment} --- time and frequency masking of
mel spectrograms --- provides the primary data augmentation strategy for
low-resource ASR and is used in our V3 architecture.

Whisper large-v3 \cite{radford2023robust}, trained on 680,000 hours of
...
\subsubsection{2.3 Script Equity and Indigenous
Scripts}\label{script-equity-and-indigenous-scripts}

Script equity in NLP has received increasing attention.
\citet{doumbouya2021} (nicolingua) established the largest public N'Ko
text corpus. \citet{tonja2023natural} surveys NLP for Ethiopic/Ge'ez, a
...
Layer analysis methodology follows \citet{ng2024rys} (Revisit Your
Shoulders), who showed that duplicating transformer layers in a model's
reasoning zone can improve mathematical performance by 17.72\% on
...
circuits. LoRA fine-tuning \cite{hu2022lora} provides the adaptation
mechanism for all LLM experiments.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{3. Activation Profiling: The N'Ko Brain
Scan}\label{activation-profiling-the-nko-brain-scan}

\subsubsection{3.1 Experimental Setup}\label{experimental-setup}

\textbf{Model.} We use Qwen2-72B-Instruct quantized to 4-bit NF4 on an
A100 80GB (Vast.ai, \$0.89/hr). The model has 81 layers (1 embedding +
80 transformer blocks), hidden dimension \(d = 8192\).

\textbf{Data.} We construct 100 parallel sentence pairs, each containing
...
\textbf{Metrics.} At each layer \(l\) with hidden state matrix
...
\textbf{Shannon Entropy} (treating normalized absolute activations as a
...
\subsubsection{3.2 Results: The Translation
Tax}\label{results-the-translation-tax}

\textbf{Table 1: L2 Norm by Layer (English vs.~N'Ko, Qwen2-72B-Instruct
base)}
...
\textbf{Table 2: Shannon Entropy by Layer}
...
\textbf{Table 3: Kurtosis by Layer (showing distribution peakedness)}
...
at the output represents an 85.8\% deficit, meaning the model's final
...
\textbf{Sparsity:} At the embedding layer, English sparsity is 13.8\%
(few near-zero activations) versus 34.5\% for N'Ko (more than twice as
many inactive dimensions). The model has not learned to use most of its
8,192 embedding dimensions for N'Ko tokens.

\subsubsection{3.3 Circuit Duplication
Analysis}\label{circuit-duplication-analysis}

Following the RYS methodology \cite{ng2024rys}, we test whether N'Ko
reasoning can be amplified by duplicating transformer layers, analogous
to the 17.72\% English math improvement reported in the original work.

\textbf{Configuration space.} We test 55 configurations: starting layer
...
performance (N'Ko score \textgreater{} English score). The difference
...
\subsubsection{3.4 Three-Zone Failure
Analysis}\label{three-zone-failure-analysis}

The activation profiles reveal three structurally distinct failure
zones:

\textbf{Zone 1: Comprehension Failure (Layers 0--10).} At the embedding
layer, N'Ko sparsity is 34.5\% versus 13.8\% for English. The model has
...
\textbf{Zone 2: Reasoning Vacuum (Layers 10--56).} The L2 ratio is
...
\textbf{Zone 3: Incoherent Output (Layers 56--80).} In the final layers,
kurtosis deficit worsens from \textasciitilde76\% to 85.8\%. The model,
...
\subsection{4. LLM Adaptation: Closing the Translation
Tax}\label{llm-adaptation-closing-the-translation-tax}

\subsubsection{4.1 Training Pipeline}\label{training-pipeline}
...
\textbf{Stage 1: Continued Pre-Training (CPT).} 17,360 text-completion
...
\textbf{Stage 2: Supervised Fine-Tuning (SFT).} 21,240
...
\textbf{Stage 3: BPE-Aware Training.} 25,100 examples generated from BPE
...
\textbf{Table 4: Training Configuration}
...
\textbf{Table 5: LLM Adaptation Results (frozen 100+100 evaluation set)}
...
\textbf{Translation Tax} & \textbf{2.90x} & \textbf{0.70x} &
\textbf{0.70x} & \textbf{-76\%} \\
\end{longtable}
}
...
\textbf{Mode collapse note.} The V3 model trained on 92,184 examples
...
\subsection{5. Audio-to-N'Ko ASR}\label{audio-to-nko-asr}

\subsubsection{5.1 The Phonetic Transparency
Hypothesis}\label{the-phonetic-transparency-hypothesis}
...
\textbf{Hypothesis:} Given equal model capacity and training data,
...
\subsubsection{5.2 The Cross-Script
Bridge}\label{the-cross-script-bridge}
...
\textbf{Latin → IPA:} Rule-based with digraph priority resolution.
...
\textbf{IPA → N'Ko:} Bijective lookup table over the full IPA inventory
...
\textbf{Six bugs found and fixed during development:}
...
\item
  ...
  \texttt{unicodedata.normalize(\textquotesingle{}NFD\textquotesingle{},\ text)}
...
In practice, 12--18\% of bridge outputs fail FSM validation (Section
...
\subsubsection{5.3 Architecture Evolution}\label{architecture-evolution}

\textbf{Training Data.} 37,306 audio clips from bam-asr-early
...
\textbf{V1: BiLSTM CTC (5.4M parameters)}
...
V1 result: 56\% CER, 91.5\% WER, validation loss 0.143. The BiLSTM lacks
sufficient temporal modeling capacity for the long-range phoneme context
in connected speech.

\textbf{Architecture Search (28 configurations)}
...
\textbf{Table 6: Architecture Search Results}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1311}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1967}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0820}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0820}}
...
\textbf{Transformer} & \textbf{512} & \textbf{4} & \textbf{4x} &
\textbf{45.7\%} & \textbf{78.6\%} & \textbf{0.121} \\
...
\textbf{V3: Transformer Fullpower (46.9M parameters)}
...
\tightlist
\item
  \textbf{Hidden dimension 768} (up from 512) increases model capacity
  while remaining within RTX 4090 memory budget at batch size 32.
\item
  \textbf{12 attention heads} with \(d_\text{head} = 64\), standard for
  768-dimensional models.
\item
  \textbf{6 Transformer layers} (up from 4) adds representational depth
  without proportionally increasing computation.
\item
  \textbf{4x downsampling only} (versus 16x in V1) via a single Conv1d
  with stride 4, preserving fine temporal resolution.
\item
  \textbf{GELU activation} in the projection head follows standard
  transformer practice.
\end{itemize}

\textbf{SpecAugment \cite{park2019specaugment}:} Applied during training
...
\textbf{Training schedule:} 5-epoch linear warmup, then cosine learning
...
V3 result: \textbf{33\% CER, 70\% WER}, validation loss 0.022. The
23-point CER improvement over V1 (56\% → 33\%) is driven primarily by
self-attention's ability to form long-range phoneme context
representations and the additional depth.

\textbf{V4: Whisper LoRA (in progress)}
...
\subsubsection{5.4 Finite-State Machine
Post-Processing}\label{finite-state-machine-post-processing}
...
99\% of sequences pass the FSM without correction. On random N'Ko
character sequences (same alphabet), only 19\% pass --- validating that
the FSM captures genuine phonotactic structure rather than trivial
constraints.

\textbf{Throughput.} FSM validation adds negligible overhead to CTC
...
\subsubsection{5.5 Results}\label{results-1}

\textbf{Table 7: Main ASR Results}
...
V3 Transformer & 46.9M & \textbf{33.0\%} & \textbf{70.0\%} &
\textbf{0.022} & \$5 \\
V4 Whisper LoRA & \textasciitilde50M & {[}in progress{]} & {[}in
progress{]} & {[}in progress{]} & \textasciitilde\$6 \\
\emph{MALIBA-AI v3 (Latin out)} & \emph{2B} & \emph{n/a (Latin script)}
& \emph{45.73\%} & \emph{n/a} & \emph{---} \\
\end{longtable}
}

\textbf{Sample Predictions (V3 model, epoch 200):}
...
\subsection{6. The Circuit Connection: Two Threads, One
Finding}\label{the-circuit-connection-two-threads-one-finding}
...
\textbf{Finding 1: The LLM failure is data starvation, not architectural
incapacity.} The 2.90x translation tax, the 0/55 circuit configurations,
the 85.8\% kurtosis deficit --- these numbers all have the same cause.
...
\textbf{Finding 2: The FSM replaces what the LLM could not learn.} The
...
guarantees 100\% phonotactic validity --- something the fine-tuned LLM
achieves at 99.8\% without the FSM constraint (after sufficient
...
\textbf{Finding 3: Phonetic transparency helps CTC in ways it cannot
...
2B-parameter system, achieves 45.73\% WER on Latin output. Our
46.9M-parameter V3 achieves 33\% CER (character-level, equivalent to
approximately 70\% WER word-level). The 43x smaller model outperforms on
a character error metric that is strictly more fine-grained. The
structural advantage is real.

\textbf{Finding 4: Self-attention enables the circuit formation that
...
56\% CER. The V3 Transformer at 46.9M achieves 33\% CER. The delta is
not entirely attributable to scale: a 768-dimensional BiLSTM at
equivalent scale achieves \textasciitilde58\% CER (from the architecture
...
\textbf{Finding 5: The bridge recovers what colonialism encoded away.}
...
\textbf{33\% CER is high for production ASR.} The best reported English
ASR systems achieve sub-5\% WER; even for low-resource African
languages, the research community targets below 20\% WER as a practical
threshold. Our 33\% CER corresponds to approximately 70\% WER, which
...
\textbf{Round-trip WER includes bridge conversion error.} The 70\% WER
...
Pure N'Ko CER (33\%) is a cleaner measure of ASR quality, but WER is the
field-standard metric for comparison with MALIBA-AI v3.

\textbf{Training data is Bambara only.} The bam-asr-early corpus
...
\textbf{Greedy CTC decoding.} V1 through V3 use greedy argmax decoding.
...
\textbf{Tone marking deficit.} The bam-asr-early corpus uses Latin
...
\textbf{Training data volume.} 37 hours of labeled speech is modest.
Published research \cite{data_scaling_2024} suggests 50 hours as a
practical minimum for African language ASR with WER below 13\%. The
...
circuits (0/55 configurations), the 85.8\% kurtosis deficit --- these
...
decoder, operating on frozen Whisper features, achieves 33\% CER for \$5
...
Antoun, W., Baly, F., and Hajj, H. (2020). AraBERT: Transformer-based
model for Arabic language understanding. In \emph{LREC Workshop on
...
Barrault, L., et al.~(2023). WMT 2023 shared task: Machine translation
for N'Ko. In \emph{Proceedings of the Eighth Conference on Machine
Translation (WMT)}. ACL Anthology 2023.wmt-1.34.

Coulibaly, A., et al.~(2025). Bayelemabaga: A Bambara-French parallel
corpus for machine translation. In \emph{Proceedings of NAACL 2025}. ACL
Anthology 2025.naacl-long.602.

Dossou, B. F. P., Tonja, A. L., et al.~(2022). AfroLM: A self-active
learning-based multilingual pretrained language model for 23 African
languages. In \emph{SustaiNLP Workshop at EMNLP}. ACL Anthology
2022.sustainlp-1.11.

Doumbouya, M., et al.~(2021). Using Radio Archives for Low-Resource
Speech Recognition: Towards an Automatic Transcription of Bambara Radio
Broadcasts. In \emph{Proceedings of NAACL}. {[}nicolingua corpus{]}

Graves, A., Fernandez, S., Gomez, F., and Schmidhuber, J. (2006).
Connectionist temporal classification: Labelling unsegmented sequence
data with recurrent neural networks. In \emph{Proceedings of ICML 2006}.

Hsu, W.-N., Bolte, B., Tsai, Y.-H. H., Lakhotia, K., Salakhutdinov, R.,
and Mohamed, A. (2021). HuBERT: Self-supervised speech representation
learning by masked prediction of hidden units. In \emph{IEEE/ACM TASLP}.
...
Kakwani, D., et al.~(2020). IndicNLPSuite: Monolingual corpora,
evaluation benchmarks and pre-trained multilingual language models for
Indian languages. In \emph{Findings of EMNLP}.

MALIBA-AI. (2024). Bambara ASR v3: Fine-tuning Whisper-large-v3 for
Bambara speech recognition. Hugging Face model card:
MALIBA-AI/bambara-asr-v3.

Magueresse, A., Carles, V., and Heetderks, E. (2020). Low-resource
languages: A review of past work and future challenges for language
technology. In \emph{Proceedings of the 1st Workshop on NLP for Positive
Impact (ACL)}.

Ng, D. N. (2024). Revisit Your Shoulders: A Circuit Analysis of
Transformer Layers for Reasoning Enhancement. arXiv preprint.

Park, D. S., et al.~(2019). SpecAugment: A simple data augmentation
method for automatic speech recognition. In \emph{Interspeech 2019}.

Pfeiffer, J., et al.~(2020). AdapterHub: A framework for adapting
transformers. In \emph{Proceedings of EMNLP 2020 (Demo)}.

Radford, A., Kim, J. W., Xu, T., Brockman, G., McLeavey, C., and
Sutskever, I. (2023). Robust speech recognition via large-scale weak
supervision. In \emph{Proceedings of ICML 2023}.

RobotsMali. (2024). bam-asr-early: Bambara automatic speech recognition
early dataset. Hugging Face dataset: RobotsMali/bam-asr-early. License:
CC-BY-4.0.

Tonja, A. L., et al.~(2023). Natural language processing in Ethiopian
languages: Current state, challenges, and opportunities. In
\emph{AfricaNLP Workshop at ACL 2023}.

Unicode Consortium. (2006). N'Ko block: U+07C0--U+07FF. \emph{The
Unicode Standard}, Version 5.0+.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\emph{Submitted for consideration at ACL/EMNLP 2026.} \emph{Code,
models, and evaluation framework:
https://github.com/Diomandeee/nko-brain-scanner} \emph{Total compute
