...
% ===================================================================
\begin{center}
{\LARGE\bfseries From Dead Circuits to Living Speech:\\[0.3em]
Activation Profiling, Script-Native Architecture Search,\\[0.3em]
and Finite-State Phonotactics for \nko{} Automatic Speech Recognition}

\vspace{1.5em}

{\large Mohamed Diomande}\\[0.3em]
{\normalsize Independent Researcher}\\[0.2em]
{\normalsize\texttt{contact@mohameddiomande.com}}
...
\begin{equation}
S(h_l) = \frac{|\{i : |h_{l,i}| < \varepsilon\}|}{d}, \qquad \varepsilon = 0.01 \cdot \max_i(|h_{l,i}|)
\label{eq:sparsity}
\end{equation}
...
\begin{table}[ht]
\centering
\caption{L2 norm by layer for English and \nko{} on Qwen2-72B-Instruct (base model, pre-fine-tuning). The ratio column shows the multiplicative factor by which English activation magnitude exceeds \nko{} at each layer.}
\label{tab:l2norm}
\begin{tabular}{@{}lccc@{}}
\toprule
Layer & English $\|h\|_2$ & \nko{} $\|h\|_2$ & Ratio (EN/NK) \\
...
\begin{table}[ht]
\centering
\caption{LLM adaptation results on the frozen 100+100 evaluation set. The translation tax is the ratio of \nko{} perplexity to English perplexity. PPL denotes perplexity; Top-1 Acc denotes next-token prediction accuracy; Token Acc denotes per-token accuracy.}
...
\begin{equation}
\mathcal{L}_{\text{CTC}} = -\log P(y \mid x) = -\log \sum_{\pi \in \mathcal{B}^{-1}(y)} \prod_{t=1}^{T} p(\pi_t \mid x)
\label{eq:ctc}
\end{equation}
...
\begin{table}[ht]
\centering
\caption{Architecture search results for eleven representative configurations. The winning configuration (Transformer, $d=512$, $L=4$, 4x downsampling) is shown in bold. All configurations use the same training data, feature extraction, and hyperparameter schedule.}
...
\textbf{Transformer} & \textbf{512} & \textbf{4} & \textbf{4x} & \textbf{45.7\%} & \textbf{78.6\%} & \textbf{0.121} \\
...
\begin{equation}
\text{Whisper}_{\text{frozen}}(x) \to \text{Linear}(1280, 768) \xrightarrow{\text{GELU}} \text{Conv1d}(\text{stride}=4) \to \text{Transformer}_6(d=768, h=12) \to \text{LN} \to \text{Linear}(768, 66)
\label{eq:v3}
\end{equation}
...
\begin{table}[ht]
\centering
\caption{V3 training loss curve progression. Qualitative observations describe the model's output at each stage. The transition from repeated characters (epoch 1) to multi-word sequences (epoch 76) illustrates the progressive formation of phoneme context representations by the transformer decoder.}
...
\textsc{Start} & space/punct & \textsc{Start} & Word boundary \\
\textsc{Onset} & $v \in V$ & \textsc{Nucleus} & CV onset complete \\
\textsc{Onset} & $c \in C$ & --- (reject) & No consonant clusters \\
\textsc{Nucleus} & $n \in \{m, n, ng\}$ & \textsc{Coda} & Nasal coda \\
\textsc{Nucleus} & $v \in V$ & --- (reject) & No hiatus \\
\textsc{Nucleus} & $c \in C \setminus \{m,n,ng\}$ & \textsc{Onset} & New syllable \\
\textsc{Nucleus} & space/punct & \textsc{Start} & Word boundary \\
\textsc{Coda} & space/punct & \textsc{Start} & Word boundary \\
...
\begin{table}[ht]
\centering
\caption{Main ASR results across all architecture versions. Parameters count trainable parameters only (the Whisper encoder is frozen in V1--V3). MALIBA-AI v3 is shown for reference but targets Latin-script output and uses a different evaluation protocol.}
...
V4 Whisper LoRA & $\sim$50M & \multicolumn{3}{c}{[in progress]} & $\sim$\$6 \\
\emph{MALIBA-AI v3 (Latin)} & \emph{2B} & \emph{n/a} & \emph{45.73\%} & \emph{n/a} & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\bibitem{maliba2024bambara}
MALIBA-AI (2024).
Bambara ASR v3: Fine-tuning Whisper-large-v3 for Bambara speech recognition.
Hugging Face model card: MALIBA-AI/bambara-asr-v3.

\bibitem{bambara_survey_2026}
MALIBA-AI (2026).
A survey of Bambara automatic speech recognition systems.
Technical report, MALIBA-AI.

\bibitem{robotsmali2024}
RobotsMali (2024).
bam-asr-early: Bambara automatic speech recognition early dataset.
Hugging Face dataset: RobotsMali/bam-asr-early. License: CC-BY-4.0.

\bibitem{bayelemabaga2025}
Coulibaly, A., et al. (2025).
Bayelemabaga: A Bambara-French parallel corpus for machine translation.
In \emph{Proceedings of NAACL 2025}. ACL Anthology 2025.naacl-long.602.

\bibitem{wmt2023nko}
Barrault, L., et al. (2023).
WMT 2023 shared task: Machine translation for N'Ko.
In \emph{Proceedings of the Eighth Conference on Machine Translation}. ACL Anthology 2023.wmt-1.34.

\bibitem{doumbouya2021}
Doumbouya, M., et al. (2021).
Using radio archives for low-resource speech recognition: Towards an automatic transcription of Bambara radio broadcasts.
...
\bibitem{ng2024rys}
Ng, D.\,N. (2024).
Revisit Your Shoulders: A circuit analysis of transformer layers for reasoning enhancement.
arXiv preprint.

\bibitem{tonja2023natural}
Tonja, A.\,L., et al. (2023).
Natural language processing in Ethiopian languages: Current state, challenges, and opportunities.
In \emph{AfricaNLP Workshop at ACL 2023}.

\bibitem{data_scaling_2024}
Magueresse, A., Carles, V., and Heetderks, E. (2020).
Low-resource languages: A review of past work and future challenges for language technology.
In \emph{Proceedings of the 1st Workshop on NLP for Positive Impact (ACL)}.

\bibitem{dossou2022afrolm}
Dossou, B.\,F.\,P., Tonja, A.\,L., et al. (2022).
AfroLM: A self-active learning-based multilingual pretrained language model for 23 African languages.
In \emph{SustaiNLP Workshop at EMNLP}. ACL Anthology 2022.sustainlp-1.11.

\bibitem{kakwani2020indicnlp}
Kakwani, D., et al. (2020).
...
In \emph{Proceedings of EMNLP 2020 (Demo)}.

\bibitem{antoun2020arabert}
Antoun, W., Baly, F., and Hajj, H. (2020).
AraBERT: Transformer-based model for Arabic language understanding.
In \emph{LREC Workshop on Open-Source Arabic Corpora and Processing Tools}.

\bibitem{unicode2006nko}
Unicode Consortium (2006).
N'Ko block: U+07C0--U+07FF.
\emph{The Unicode Standard}, Version 5.0+.
